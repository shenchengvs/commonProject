...
import com.holley.platform.common.constants.InitParams;
import com.holley.platform.common.constants.TaskStatusEnum;
...
public class ThreadPoolUtil {
    ...
    private static ThreadPoolExecutor getPool() throws XPathExpressionException, ParserConfigurationException, IOException, SAXException {
        if (pool == null) {
            int corePoolSize = Integer.parseInt(InitParams.getThreadPoolParam(COREPOOLSIZE).toString());
            int maximumPoolSize = Integer.parseInt(InitParams.getThreadPoolParam(MAXIMUMPOOLSIZE).toString());
            int keepAliveTime = Integer.parseInt(InitParams.getThreadPoolParam(KEEPALIVETIME).toString());
            String queueType = InitParams.getThreadPoolParam(QUEUETYPE).toString();
            if ("SynchronousQueue".equals(queueType)) {
                ...
                int queueSize = Integer.parseInt(InitParams.getThreadPoolParam(QUEUESIZE).toString());
    ...
    public static List<ThreadInfo> getThreadInfoList(String moduleName) {
        ...
        Iterator<String> iterator = CachedThreadInfoUtil.THREADINFO.keySet().iterator();
    ...
    private static void clearThreadInfo() {
        ...
        while (iterator.hasNext()) {
            ...
            if (value.getStatus() == TaskStatusEnum.FINISH.getValue() && value.getEnd().compareTo(twoHourAgo) < 0) {// 清除
                                                                                                                    // 两小时前的已结束的线程信息
