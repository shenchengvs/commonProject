...
public class DateUtil extends DateUtils {
     ...
     * @return
     */
    public static Date getCircleEndTime(Date startTime, CircleTypeEnum circleTypeEnum, int offset) {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(getWeekValue(new Date()));
    ...
    public static List<Date> getLoadDateBetween(Date date1, Date date2, int points) {
        ...
        while (date1.compareTo(date2) < 0) {
            if (points == 24) {
                ...
                date1 = DateUtils.addMinutes(date1, 5);
            } else {
                return list;
            }
            if (!list.contains(date1)) { // 避免重复数据
     ...
     * @return
     */
    public static List<String> getTimePointList(int circletypeid, int maxMonthDay) {
        List<String> datatimeList = new ArrayList<String>();
        if (CircleTypeEnum.HOUR.getValue() == circletypeid) {// 逐时
            for (int i = 0; i < 24; i++) {
                if (i <= 9) {
        ...
        } else if (CircleTypeEnum.DAY.getValue() == circletypeid) {// 逐日
            for (int i = 1; i <= maxMonthDay; i++) {
                if (i <= 9) {
                    datatimeList.add("0" + String.valueOf(i));
                } else {
        ...
        } else if (CircleTypeEnum.MONTH.getValue() == circletypeid) {// 逐月
            for (int i = 1; i <= 12; i++) {
                if (i <= 9) {
                    datatimeList.add("0" + String.valueOf(i));
                } else {
    ...
    public static String getTodayWeekStr() {
        ...
        String result = StringUtil.AddjustLength(String.valueOf(monthday), 2, "0") + StringUtil.AddjustLength(String.valueOf(weekday), 2, "0");
        return result;
    }
}
