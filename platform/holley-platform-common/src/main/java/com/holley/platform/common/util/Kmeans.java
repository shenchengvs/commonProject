...
import com.holley.platform.common.file.FileUtil;
...
public class Kmeans {
    ...
    private void kmeans() {
        ...
        while (true) {
            count++;
            System.out.println("第" + count + "次聚类开始");
            clusterSet();
    ...
    private void clusterSet() {
        ...
        for (int i = 0; i < dataSetLength; i++) {
            // System.out.println("clusterSet" + i);
    ...
    private void initDataSet() {
        ...
        float[][] dataSetArray = new float[][] { { 8, 2 }, { 3, 4 }, { 2, 5 }, { 4, 2 }, { 7, 3 }, { 6, 2 }, { 4, 7 }, { 6, 3 }, { 5, 3 }, { 6, 3 }, { 6, 9 }, { 1, 6 }, { 3, 9 },
                                                 { 4, 1 }, { 8, 6 } };
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<String> ltList = FileUtil.readLinesFromFile("F:\\360yunpan\\南方电网电动汽车项目\\05系统成果\\20151115南方电网鉴定\\原始数据\\all_data.txt", -1);

        if (ltList != null) {
            for (String lt : ltList) {
                String[] ltsp = StringUtils.split(lt, "\t");
                if (ltsp != null && ltsp.length == 2) {
                    dataSet.add(new float[] { new Float(ltsp[0].trim()), new Float(ltsp[1].trim()) });
                }
            }
        ...
        k.printDataArray(center, "center");
